...
(function() { 
    ...
    // const selectVisibleBtn = document.getElementById('selectVisibleBtn'); // Удалено
    ...
    function enablePhaseControls(enable, refreshButtonAlso = true) {
        ...
        // Кнопки "Переключить все" и "По умолчанию"
        ...
        // Кнопка "Применить изменения" управляется отдельно через updatePendingStatus
    ...
    function renderPhaseTree(allPhaseData) {
        ...
        sortedPhaseNames.forEach(phaseName => {
            if (phaseExpandedState.hasOwnProperty(phaseName)) {
                ...
                newPhaseExpandedState[phaseName] = sortedPhaseNames.length === 1; 
            }
        });
            ...
            case 'loadInitialState':
                ...
                if (message.error) { 
                    ...
                    Object.keys(testDataByPhase).forEach(phaseName => {
                        if (!phaseExpandedState.hasOwnProperty(phaseName)) { // Инициализируем только если нет сохраненного
                             phaseExpandedState[phaseName] = (Object.keys(testDataByPhase).length === 1);
                        }
    ...
    // Удаляем обработчик для selectVisibleBtn, т.к. кнопка удалена
